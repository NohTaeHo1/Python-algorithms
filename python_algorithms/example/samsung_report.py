...
from python_algorithms.context.model.data_model import DataModel
...
class SamsungReport:
    ...
    def __init__(self):
        self.okt = Okt()
        self.data = DataModel()
        self.data.dname = 'C:\\Users\\bitcamp\\docker_pod\\python-algorithms\\Python-algorithms\\python_algorithms\\example\\data\\'
        self.data.fname = 'kr-Report_2018.txt'
        self.text = '코드잇에 오신 걸 환영합니다'
        self.result = ""
        self.nouns = []
        self.stopwords = []
        self.morphemes = []
    ...
    def noun_embedding(self):
        ...
        for token in tokens:
            ...
            _ = [j[0] for j in pos if j[1] == 'Noun'] # 사전과 비교하면서 명사만 살림
            if len(''.join(_)) > 1:
    ...
    def draw_wordclound(self):
        freqtext = pd.Series(dict(FreqDist(self.morphemes))).sort_values(ascending=False)
        ic(freqtext[:10])
        wcloud = WordCloud(f'{self.data.dname}D2Coding.ttf', relative_scaling=0.2, background_color='white').generate_from_frequencies(freqtext)
...
punkt 또한 문장 구조를 학습한 일종의 모델로, 어떤 것이 약어에 쓰이는 "."이고(Ex : Ph.D.), 어떤 것이 마침표인지 학습이 되어있다. 
문장을 기본적으로 마침표를 기준으로 나누되, Ph.D., Saint., Professor., 와 같은 약어(Abbreviation)는 Known abbreviation으로 학습하여 한 단어로 취급하는 방식이다.

하지만 이러한 punkt모델에도 치명적인 단점이 있는데, 모든 약어를 학습하지 못했다보니, 
Vol. 13, Apr. 13 과 같은 표현 및 U.S. Pat. No. 134 과 같은 복잡한 약어는 Known abbreviation이 아니여서 모두 나눠져버린다는 것이다.


<JDK-19 에서 JPype + KoNLPy 설치>
